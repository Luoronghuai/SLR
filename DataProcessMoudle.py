...
def Word2Id(trainLabelPath, validLabelPath, testLabelPath, dataSetName):
    vocab_save_path = "/root/word_idx/vocab_data.pkl"
    if os.path.exists(vocab_save_path):
        print("pickle")
        with open(vocab_save_path, 'rb') as f:
            data = pickle.load(f)
            word2idx = data['word2idx']
            wordSetNum = data['wordSetNum']
...
def collate_fn(batch):
    ...
    for sample in batch:
        vid = sample["video"]
        collated["videoLength"].append(
            torch.LongTensor([np.ceil(len(vid) / total_stride) * total_stride + 2 * left_pad]))
